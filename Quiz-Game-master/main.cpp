...
void round(bool AskedQ[30], int &current_score)
{
    ...
    for (unsigned int QNum = 1; QNum <= 6; QNum++)
    {
        ...
        cout << "Q" << QNum << ": " << QList[r].first.first << endl
             << QList[r].first.second << endl;
        cin >> user_input;
...
int main()
{
    ...
    cin >> username;
    cout << endl
         << username << "'s highest score is: " << score_list[username] << "\nReady To Break It?!" << endl
         << endl;

    cout << "Choose QuIzGaMe field:" << endl
         << "1-SPORTS \n2-BIOLOGY \n3-HISTORY \n4-Mental Abilities \n5-Arts" << endl;
    cin >> user_input;
    ...
    if (current_score == 6)
    {
        ...
        cout << "Do you want to move to the final round?" << endl
             << "1-)YES 2-)any other number than 1 will be considered no" << endl;
        cin >> user_input;
        if (user_input == "1")
        {
            QList.clear();
            cout << "Choose QuIzGaMe field for this round:" << endl
                 << "1-SPORTS \n2-BIOLOGY \n3-HISTORY \n4-Math \n5-Geography" << endl;
            cin >> user_input;
            while (user_input.size() > 1 || (user_input > "5" || user_input < "1"))
            {
